...
int main(int argc, char *argv[]) {
    ...
    if (err == CL_DEVICE_NOT_FOUND) {
        printf("Error: No OpenCL devices were found", err);
        exit(1);
    }
    ...
    ctx.commands = clCreateCommandQueueWithProperties(ctx.context, ctx.device_id, 0, &err);
    if (!ctx.commands) {
